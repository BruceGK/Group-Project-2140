...
def mlsearch():
    ...
    if start == None:
        start = 0
    if int(start) > 380:
        return 'Due to server limitation, we can only process the top 400 documents in ML mode', 400
    start = int(start)
    reader = IndexReader()
    # fetch without highlight first
    result = reader.search("cord_test", query, size=400, fields=[], highlight=False)
